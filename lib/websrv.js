...
function websrv(prefix){
    ...
    app.use(async(ctx, next) => {
        ...
        if(ctx.request.body && emitLog) {
            let body = tools.isJSON(ctx.request.body) ? JSON.stringify(ctx.request.body) : ctx.request.body;
            this.emit('log', LOG_LEVELS.DEBUG, `request: ${body.length > this.maxMsg ? body.substr(0, this.maxMsg) + '...' : body}`);
        }
        ...
        if(ctx.body && emitLog){
            let body = tools.isJSON(ctx.body) ? JSON.stringify(ctx.body) : ctx.body;
            this.emit('log', LOG_LEVELS.DEBUG, `response: ${body.length > this.maxMsg ? body.substr(0, this.maxMsg) + '...' : body}`);
        }
     ...
     * @param port
     */
    websrv.prototype.start = function(port, ip = '127.0.0.1'){

        // 消息体解析
        this.app.use(bodyparser({jsonLimit: '10mb', formLimit: '200mb'}));
